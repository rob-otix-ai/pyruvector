...
impl CollectionManager {
    ...
    fn create_collection(
        ...
        // Check if collection already exists
        {
            ...
            if collections.contains_key(&name) {
                return Err(PyValueError::new_err(format!("Collection '{}' already exists", name)));
            }
        }
    ...
    fn get_collection(&self, name: String) -> PyResult<VectorDB> {
        ...
        collections.get(&collection_name)
            .cloned()
            .ok_or_else(|| PyKeyError::new_err(format!("Collection '{}' not found", collection_name)))
    }
    ...
    fn delete_collection(&self, name: String) -> PyResult<bool> {
        // Remove any aliases pointing to this collection
        {
            ...
            let aliases_to_remove: Vec<String> = aliases.iter()
    ...
    fn get_stats(&self, name: String) -> PyResult<CollectionStats> {
        ...
        let db = collections.get(&collection_name)
            .ok_or_else(|| PyKeyError::new_err(format!("Collection '{}' not found", collection_name)))?;
    ...
    fn create_alias(&self, alias: String, collection: String) -> PyResult<()> {
        ...
        // Check if collection exists
        {
            ...
            if !collections.contains_key(&collection) {
                return Err(PyKeyError::new_err(format!("Collection '{}' not found", collection)));
            }
        }
    ...
    fn list_aliases(&self) -> PyResult<Vec<(String, String)>> {
        ...
        let mut result: Vec<(String, String)> = aliases.iter()
...
mod tests {
    ...
    fn test_create_and_get_collection() {
        ...
        // Create collection
        manager.create_collection("test".to_string(), 128, None).unwrap();
    ...
    fn test_list_collections() {
        ...
        manager.create_collection("col1".to_string(), 128, None).unwrap();
        manager.create_collection("col2".to_string(), 256, None).unwrap();
    ...
    fn test_aliases() {
        ...
        manager.create_collection("original".to_string(), 128, None).unwrap();
        manager.create_alias("alias1".to_string(), "original".to_string()).unwrap();
        ...
        assert_eq!(aliases, vec![("alias1".to_string(), "original".to_string())]);
    ...
    fn test_delete_collection_removes_aliases() {
        ...
        manager.create_collection("test".to_string(), 128, None).unwrap();
        manager.create_alias("alias1".to_string(), "test".to_string()).unwrap();
        manager.create_alias("alias2".to_string(), "test".to_string()).unwrap();
